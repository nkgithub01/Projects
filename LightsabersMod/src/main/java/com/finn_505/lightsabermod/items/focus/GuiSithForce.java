...
public class GuiSithForce extends GuiScreen{
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) 
	{
		...
		this.drawString(fontRenderer, "Choke", this.width/2 - this.fontRenderer.getStringWidth("Choke")/2 - 44, this.height/2 + 43, 0xFFFFFF);
		this.drawString(fontRenderer, "Lightning", this.width/2 - this.fontRenderer.getStringWidth("Lightning")/2 + 44, this.height/2 + 43, 0xFFFFFF);
	...
	public void handleMouseInput() throws IOException 
	{
		...
		if(mouseY <= height/2- 10)
		{
			...
			{
				this.mc.player.getCapability(ForceProvider.FORCE, null).setEquippedAbility(6);
				Main.network.sendToServer(new SetForceAbility(6));
				currentPos = 1;
			}
			else if(mouseX < this.width/2)
			{
				this.mc.player.getCapability(ForceProvider.FORCE, null).setEquippedAbility(5);
				Main.network.sendToServer(new SetForceAbility(5));
				currentPos = 2;
			}
		}
