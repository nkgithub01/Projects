...
public class ForceSync implements IMessage{
	...
	public ForceSync(int blocking, int hate, int maxForce, int currentAbility, boolean passives, boolean canJump, int force, int maxPassive, 
	...
	public static class ForceSyncHandler implements IMessageHandler<ForceSync, IMessage>
	{

		@Override
		public IMessage onMessage(ForceSync message, MessageContext ctx) {
			IThreadListener mainThread = FMLCommonHandler.instance().getWorldThread(ctx.getClientHandler());
			mainThread.addScheduledTask(new Runnable() {
				
				@Override
				public void run() 
				{
					IForce player = Minecraft.getMinecraft().player.getCapability(ForceProvider.FORCE, null);
